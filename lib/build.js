  ...
  fs.readFile(build.options.root + src, 'utf-8', function (err, contents) {
    ...
    try {
      ...
      if (!currentModule) {
        ...
        currentModule.register(moduleName);
        if (currentModule.options.amd) {
          contents = contents.replace('define(', "define('" + moduleName + "', ");
        } else if (currentModule.options.pub) {
    ...
    currentModule.emit('build', moduleName, contents);
...
Build.prototype.compile = function () {
  ...
  lodash.each(mods, function (mod) {
    modList[mod.getModuleName()] = mod.getDependencies();
  });
