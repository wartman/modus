...
modus.Loader.prototype.loadBuilding = function (moduleName) {
  ...
  fs.readFile(build.options.root + src, 'utf-8', function (err, contents) {
    ...
    var factory = new Function('modus, module, define', contents);
    // If this has config info, try to parse it.
    var getConfig = /modus\.config\(\{([\s\S\r\n'"\{\}]+?)\}\)/g
    // Config might be defined by modus.main too.
    var getMainConfig = /modus\.main\(\{([\s\S\r\n'"\{\}]+)\},\s?function/g;
    var config = null;
    var currentModule;
    contents = contents.replace(getConfig, function (match, cfg) {
      build.outputConfig(cfg);
      return "";
    });
    contents = contents.replace(getMainConfig, function (match, cfg) {
      build.outputConfig(cfg);
    ...
    try {
      ...
      // Run the factory.
      factory(modus, modus.module, modus.define);
      global.define = null;
      ...
      if (!currentModule) {
        ...
        if (currentModule.getModuleMeta('isAmd')) {
          ...
          contents = contents.replace('module(', "module('" + moduleName + "', ");
...
Build.prototype.compile = function (next) {
  ...
  var modusRaw = fs.readFileSync(__dirname + '/../dist/modus.min.js', 'utf-8');
