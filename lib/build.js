...
var _buildLoad = function (moduleName, next, error) {
  ...
  fs.readFile(build.options.root + src, 'utf-8', function (err, contents) {
    ...
    try {
      ...
      var currentModule = modus.getModule(moduleName);
      ...
      var currentModule = modus.module(moduleName, function () {});
    }
    ...
    // Run build events.
    modus.events.emit('build:load', currentModule, contents, build);
    currentModule.emitModuleEvent('build:load', currentModule, contents, build);
...
Build.prototype.compile = function () {
  ...
  // 'build:compileBefore' is a good hook to use if you need to investigate
  // any modules to, say, load AJAX docs.
  modus.events.emit('build:compileBefore', this.getOutput(), this);
  mods = modus.getModule();
  ...
  // Hook to mess with the finished script, if need be.
  modus.events.emit('build:compileAfter', this._compiled, this);
